...
const g=i.compile(m),a=t.map(e=>g(e)).join("");console.log(" markup:",a);u.innerHTML=a;console.log('%c Додаємо/Редагуємо/Видаляємо розробників в секції "Наша команда" ',"color: white; background-color: #D33F49");//! 1.1 Взяти з localStorage дані які вже у нас є! 
...
document.querySelector(".our-developers-list");const p=document.querySelector(".our-developers__button"),b=document.querySelector(".form-add-edit-developer__title"),w=document.querySelector("[data-modal-add-edit-developer]"),h=document.querySelector(".modal-add-edit-developer__button-delete"),l=document.querySelector(".form-add-edit-developer");//! 2.2 Змінні для поточної роботи
//! 3 надати слухачів подій
p.addEventListener("click",v);l.addEventListener("submit",L);//! 4 блок функцій 
...
b.textContent="Додати розробника",h.style.display="none";//! 4.1.2 відкриття модального вікна з формою для збирання даних
...
function j(){console.log("відкривання/закривання модального вікна для додавання/редагування розробника"),w.classList.toggle("is-hidden"),document.body.classList.toggle("no-scroll")}//! підтвердження даних в формі модальних  вікон ДОДАТИ/РЕДАГУВАТИ 
function L(e){//! блокуємо перезавантаження сторінки 
e.preventDefault();//! 4.3.1 зібрати дані з форми по кнопці "submit" і записати ці дані в об'єкт
console.log("ДО dataDevelopersList:",o);const s={name:l.developerName.value,position:l.developerPosition.value};console.log("developerObject:",s);//! 4.3.2 додаємо новий об'єкт до масиву об'єктів
o.push(s),console.log("ПІСЛЯ  dataDevelopersList:",o)}const x=[{name:"Студія",href:"./index.html",active:!0},{name:"Портфоліо",href:"./portfolio.html",active:!1},{name:"Контакти",href:"./contacts.html",active:!1}],U={links:x},R=`{{#each links}}
