...
const w=a.compile(u),n=o.map(e=>w(e)).join("");console.log(" markup:",n);g.innerHTML=n;console.log('%c Додаємо/Редагуємо/Видаляємо розробників в секції "Наша команда" ',"color: white; background-color: #D33F49");//! 1.1 Взяти з localStorage дані які вже у нас є! 
...
document.querySelector(".our-developers-list");const v=document.querySelector(".our-developers__button"),c=document.querySelector(".form-add-edit-developer__title"),h=document.querySelector("[data-modal-add-edit-developer]"),d=document.querySelector(".modal-add-edit-developer__button-delete"),i=document.querySelector(".form-add-edit-developer"),j=document.getElementById("button-cancel"),x=document.querySelectorAll("#img-developer"),L=document.querySelector(".form-add-edit-developer__button-submit");//! 2.2 Змінні для поточної роботи
//! 3 надати слухачів подій
v.addEventListener("click",U);i.addEventListener("submit",k);j.addEventListener("click",y);x.forEach(e=>{e.addEventListener("click",R),console.log("click on:",e)});//! 4 блок функцій 
function U(){console.log("додаємо нового розробника");//! 4.1.1 заміна контенту заголовка модального вікна, приховуємо зайві елементи
c.textContent="Додати розробника",d.style.display="none";//! 4.1.2 відкриття модального вікна з формою для збирання даних
l()}function R(){console.log("Вікно редагування");//! 4.1.1 заміна контенту заголовка модального вікна, відкриваємо необхідні елементи
c.textContent="Редагування розробника",L.textContent="Редагувати",d.style.display="block";//! при натисканні на розробника треба, щоб підставлялось його зображення
l()}//! Закривання модально вікна при стоворенні нового розробника 
function y(){l()}//! відкривання/закривання модального вікна для додавання/редагування розробника
function l(){console.log("відкривання/закривання модального вікна для додавання/редагування розробника"),h.classList.toggle("is-hidden"),document.body.classList.toggle("no-scroll")}//! підтвердження даних в формі модальних  вікон ДОДАТИ/РЕДАГУВАТИ 
function k(e){//! блокуємо перезавантаження сторінки 
...
l()}const _=[{name:"Студія",href:"./index.html",active:!0},{name:"Портфоліо",href:"./portfolio.html",active:!1},{name:"Контакти",href:"./contacts.html",active:!1}],D={links:_},S=`{{#each links}}
