...
const x="http://localhost:3000/",S="developers",h=`${x}${S}`;console.log("url:",h);const E=document.querySelector(".our-developers-list");//! var 1
function q(){$().then(e=>M(e)).catch(e=>console.log(e))}function $(){return fetch(h).then(e=>e.json())}//!var2
//! генеруємо HTML за допомогою Handlebars
const B=p.compile(D);function M(e){const t=e.map(d=>B(d)).join("");E.innerHTML=t}q();//! по натиску на кнопку додати розробника: 
...
const T="http://localhost:3000/",C="developers",r=`${T}${C}`;console.log("url:",r);let u="",l=null,s="";//! Зчитуємо дані з json-server та зберігаємо їх в Масив Об'єктів: dataDevelopersList
function A(){return fetch(r).then(e=>e.json()).then(e=>l=e)}A();console.log("dataDevelopersList:",l);//! 2️⃣ Пошук DOM
const I=document.querySelector(".our-developers-list"),U=document.querySelector(".our-developers__button"),g=document.querySelector(".form-add-edit-developer__title"),R=document.querySelector("[data-modal-add-edit-developer]"),v=document.querySelector(".modal-add-edit-developer__button-delete"),H=document.querySelector(".form-add-edit-developer"),N=document.getElementById("button-cancel"),m=document.querySelector(".form-add-edit-developer__button-submit"),f=document.querySelector(".form-add-edit-developer__img"),i=document.querySelector(".form-add-edit-developer__input-developer-name"),a=document.querySelector(".form-add-edit-developer__input-developer-position"),b=document.querySelector("[data-modal-confirm-delete]"),P=document.getElementById("confirm-delete-yes"),O=document.getElementById("confirm-delete-no"),F=document.getElementById("confirm-dev-name");//! 3️⃣ Слухачі подій
U.addEventListener("click",J);H.addEventListener("submit",z);N.addEventListener("click",G);I.addEventListener("click",function(e){e.target.classList.contains("our-developers-list__img")&&W(e)});v.addEventListener("click",K);O.addEventListener("click",Q);P.addEventListener("click",Y);//! 4️⃣ Функції
//! ДОДАТИ
function J(){console.log("dataDevelopersList-addDeveloper:",l),g.textContent="Додати розробника",m.textContent="Додати",v.style.display="none",i.value="",a.value="",c()}//! РЕДАГУВАННЯ
function W(e){g.textContent="Редагування розробника",m.textContent="Редагувати",v.style.display="block",console.log("dataDevelopersList:",l),s=l.find(t=>t.name===e.target.alt),console.log("об'єкт, що редагується:",s),u=s.name,i.value=s.name,a.value=s.position,f.src=s.images.default,c()}//! ВИДАЛЕННЯ
function Y(){l=JSON.parse(localStorage.getItem("dataDevelopers")),l=l.filter(e=>e.name!==u),localStorage.setItem("dataDevelopers",JSON.stringify(l,null,2)),c(),location.reload()}//! SUBMIT ФОРМИ
function z(e){if(m.textContent==="Додати"){const o={name:i.value,position:a.value,images:{desktop:["/webstudio-vite-js/images/sample-desktop-1x.jpg 1x,","/webstudio-vite-js/images/sample-desktop-2x.jpg 2x,","/webstudio-vite-js/images/sample-desktop-3x.jpg 3x"],tablet:["/webstudio-vite-js/images/sample-tablet-1x.jpg 1x,","/webstudio-vite-js/images/sample-tablet-2x.jpg 2x,","/webstudio-vite-js/images/sample-tablet-3x.jpg 3x"],mobile:["/webstudio-vite-js/images/sample-mobile-1x.jpg 1x,","/webstudio-vite-js/images/sample-mobile-2x.jpg 2x,","/webstudio-vite-js/images/sample-mobile-3x.jpg 3x"],default:"/webstudio-vite-js/images/sample-mobile-1x.jpg"},icons:["/webstudio-vite-js/images/symboldefs.svg#instagram","/webstudio-vite-js/images/symboldefs.svg#twitter","/webstudio-vite-js/images/symboldefs.svg#facebook","/webstudio-vite-js/images/symboldefs.svg#linkedin"],date:new Date().toLocaleString()};console.log("requestBody:",o);//! ================= CREATE (POST) =================
fetch(r,{method:"POST",body:JSON.stringify(o),headers:{"Content-Type":"application/json; charset=UTF-8"}}).then(n=>n.json()).then(n=>console.log("POST:",n)).catch(n=>console.log(n))}//! мій код з помилкою 
if(m.textContent==="Редагувати"){console.log("dataDevelopersList:",l),s=l.find(o=>o.name===u),console.log("editableDeveloper:",s),s.name=i.value,s.position=a.value;//! ⭐ ФІКС КАРТИНКИ
s.images.default=f.src}//! ================= UPDATE (PUT) =================
console.log("editableDeveloper:",s);let t=s.id;console.log("developerId:",t);//! ================= UPDATE (PATCH) =================
const d={name:i.value,position:a.value};fetch(`${r}/${t}`,{method:"PATCH",body:JSON.stringify(d),headers:{"Content-Type":"application/json; charset=UTF-8"}}).then(o=>o.json()).then(o=>console.log("PATCH:",o)).catch(o=>console.log(o)),c()}//! MODAL
function c(){R.classList.toggle("is-hidden"),document.body.classList.toggle("no-scroll")}function G(){c()}//! CONFIRM DELETE
function K(e){e.preventDefault(),F.textContent=u,b.classList.remove("is-hidden"),document.body.classList.add("no-scroll")}function Q(){b.classList.add("is-hidden"),document.body.classList.remove("no-scroll")}const V=document.querySelectorAll(".our-developers-list__img"),X=e=>{e.src=e.dataset.src,e.parentElement.querySelectorAll("source").forEach(o=>{o.srcset=o.dataset.srcset})},y=new IntersectionObserver(e=>{e.forEach(t=>{t.isIntersecting&&(X(t.target),y.unobserve(t.target),console.log(`Завантажено: ${t.target.alt}`))})},{threshold:.5});V.forEach(e=>y.observe(e));const Z=[{name:"Студія",href:"./index.html",active:!0},{name:"Портфоліо",href:"./portfolio.html",active:!1},{name:"Контакти",href:"#contacts",active:!1}],ee={links:Z},te=`{{#each links}}
