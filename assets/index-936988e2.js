...
//! ДОДАТИ
function U(){v.textContent="Додати розробника",l.textContent="Додати",u.style.display="none",i.value="",a.value="",n()}//! РЕДАГУВАННЯ
function O(e){v.textContent="Редагування розробника",l.textContent="Редагувати",u.style.display="block",o=JSON.parse(localStorage.getItem("dataDevelopers"));const t=o.find(s=>s.name===e.target.alt);r=t.name,i.value=t.name,a.value=t.position,c.src=t.images.default,n()}//! ВИДАЛЕННЯ
...
function A(e){if(e.preventDefault(),o=JSON.parse(localStorage.getItem("dataDevelopers")),l.textContent==="Додати"){const t={name:i.value,position:a.value,images:{default:c.src||"./images/developers/default.jpg"},date:new Date().toLocaleString()};o.push(t)}if(l.textContent==="Редагувати"){const t=o.find(s=>s.name===r);t.name=i.value,t.position=a.value;//! ⭐ ФІКС КАРТИНКИ
