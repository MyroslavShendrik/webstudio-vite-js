...
const D="http://localhost:3000/",k="developers",m=`${D}${k}`;console.log("url:",m);const S=document.querySelector(".our-developers-list");//! var 1
function j(){E().then(e=>$(e)).catch(e=>console.log(e))}function E(){return fetch(m).then(e=>e.json())}//!var2
//! генеруємо HTML за допомогою Handlebars
const q=r.compile(L);function $(e){const t=e.map(o=>q(o)).join("");S.innerHTML=t}j();//! по натиску на кнопку додати розробника: 
...
const x="http://localhost:3000/",M="developers",p=`${x}${M}`;console.log("url:",p);let a="";//! 2️⃣ Пошук DOM
const B=document.querySelector(".our-developers-list"),C=document.querySelector(".our-developers__button"),v=document.querySelector(".form-add-edit-developer__title"),I=document.querySelector("[data-modal-add-edit-developer]"),d=document.querySelector(".modal-add-edit-developer__button-delete"),T=document.querySelector(".form-add-edit-developer"),U=document.getElementById("button-cancel"),n=document.querySelector(".form-add-edit-developer__button-submit"),c=document.querySelector(".form-add-edit-developer__img"),l=document.querySelector(".form-add-edit-developer__input-developer-name"),i=document.querySelector(".form-add-edit-developer__input-developer-position"),h=document.querySelector("[data-modal-confirm-delete]"),R=document.getElementById("confirm-delete-yes"),A=document.getElementById("confirm-delete-no"),N=document.getElementById("confirm-dev-name");//! 3️⃣ Слухачі подій
C.addEventListener("click",O);T.addEventListener("submit",J);U.addEventListener("click",F);B.addEventListener("click",function(e){e.target.classList.contains("our-developers-list__img")&&H(e)});d.addEventListener("click",W);A.addEventListener("click",Y);R.addEventListener("click",P);//! 4️⃣ Функції
//! ДОДАТИ
function O(){v.textContent="Додати розробника",n.textContent="Додати",d.style.display="none",l.value="",i.value="",s()}//! РЕДАГУВАННЯ
function H(e){v.textContent="Редагування розробника",n.textContent="Редагувати",d.style.display="block",dataDevelopersList=fetch("http://localhost:3000/developers"),console.log("dataDevelopersList:",dataDevelopersList);const t=dataDevelopersList.find(o=>o.name===e.target.alt);a=t.name,l.value=t.name,i.value=t.position,c.src=t.images.default,s()}//! ВИДАЛЕННЯ
function P(){dataDevelopersList=JSON.parse(localStorage.getItem("dataDevelopers")),dataDevelopersList=dataDevelopersList.filter(e=>e.name!==a),localStorage.setItem("dataDevelopers",JSON.stringify(dataDevelopersList,null,2)),s(),location.reload()}//! SUBMIT ФОРМИ
function J(e){if(e.preventDefault(),n.textContent==="Додати"){const t={name:l.value,position:i.value,images:{default:c.src||"./images/developers/default.jpg"},icons:["/webstudio-vite-js/images/symboldefs.svg#instagram","/webstudio-vite-js/images/symboldefs.svg#twitter","/webstudio-vite-js/images/symboldefs.svg#facebook","/webstudio-vite-js/images/symboldefs.svg#linkedin"],date:new Date().toLocaleString()};fetch(`${p}`,{method:"POST",body:JSON.stringify(t),headers:{"Content-Type":"application/json; charset=UTF-8"}}).then(o=>o.json()).then(o=>console.log("POST:",o)).catch(o=>console.log(o))}if(n.textContent==="Редагувати"){const t=dataDevelopersList.find(o=>o.name===a);t.name=l.value,t.position=i.value;//! ⭐ ФІКС КАРТИНКИ
t.images.default=c.src}localStorage.setItem("dataDevelopers",JSON.stringify(dataDevelopersList,null,2)),s()}//! MODAL
function s(){I.classList.toggle("is-hidden"),document.body.classList.toggle("no-scroll")}function F(){s()}//! CONFIRM DELETE
function W(e){e.preventDefault(),N.textContent=a,h.classList.remove("is-hidden"),document.body.classList.add("no-scroll")}function Y(){h.classList.add("is-hidden"),document.body.classList.remove("no-scroll")}const z=document.querySelectorAll(".our-developers-list__img"),G=e=>{e.src=e.dataset.src,e.parentElement.querySelectorAll("source").forEach(u=>{u.srcset=u.dataset.srcset})},f=new IntersectionObserver(e=>{e.forEach(t=>{t.isIntersecting&&(G(t.target),f.unobserve(t.target),console.log(`Завантажено: ${t.target.alt}`))})},{threshold:.5});z.forEach(e=>f.observe(e));const K=[{name:"Студія",href:"./index.html",active:!0},{name:"Портфоліо",href:"./portfolio.html",active:!1},{name:"Контакти",href:"#contacts",active:!1}],Q={links:K},V=`{{#each links}}
