...
const q="http://localhost:3000/",T="developers",f=`${q}${T}`;console.log("url:",f);const B=document.querySelector(".our-developers-list");//! var 1
function M(){C().then(e=>U(e)).catch(e=>console.log(e))}function C(){return fetch(f).then(e=>e.json())}//!var2
//! генеруємо HTML за допомогою Handlebars
const A=g.compile(j);function U(e){const t=e.map(d=>A(d)).join("");B.innerHTML=t}M();//! по натиску на кнопку додати розробника: 
...
function v(){return fetch(i).then(e=>e.json()).then(e=>a=e)}v();console.log("dataDevelopersList:",a);//! 2️⃣ Пошук DOM
const H=document.querySelector(".our-developers-list"),y=document.querySelector(".our-developers__button");console.log(y);const _=document.querySelector(".form-add-edit-developer__title"),P=document.querySelector("[data-modal-add-edit-developer]"),h=document.querySelector(".modal-add-edit-developer__button-delete"),N=document.querySelector(".form-add-edit-developer"),O=document.getElementById("button-cancel"),m=document.querySelector(".form-add-edit-developer__button-submit"),p=document.querySelector(".form-add-edit-developer__img"),l=document.querySelector(".form-add-edit-developer__input-developer-name"),n=document.querySelector(".form-add-edit-developer__input-developer-position"),b=document.querySelector("[data-modal-confirm-delete]"),F=document.getElementById("confirm-delete-yes"),J=document.getElementById("confirm-delete-no"),W=document.getElementById("confirm-dev-name");//! 3️⃣ Слухачі подій
y.addEventListener("click",Y);N.addEventListener("submit",K);O.addEventListener("click",w);H.addEventListener("click",function(e){e.target.classList.contains("our-developers-list__img")&&z(e)});h.addEventListener("click",Q);J.addEventListener("click",L);F.addEventListener("click",G);//! 4️⃣ Функції
//! ДОДАТИ
function Y(){console.log("Натиснули Додати"),o=null,r="",_.textContent="Додати розробника",m.textContent="Додати",h.style.display="none",l.value="",n.value="",p.src="/images/default.jpg",u()}//! РЕДАГУВАННЯ
function z(e){_.textContent="Редагування розробника",m.textContent="Редагувати",h.style.display="block",console.log("dataDevelopersList:",a),o=a.find(t=>t.name===e.target.alt),console.log("об'єкт, що редагується:",o),r=o.name,l.value=o.name,n.value=o.position,p.src=o.images.default,u()}//! ВИДАЛЕННЯ
async function G(){try{const e=o.id;console.log("DELETE:",`${i}/${e}`);const t=await fetch(`${i}/${e}`,{method:"DELETE"});if(!t.ok)throw new Error(`Помилка видалення: ${t.status}`);L(),w(),await v(),location.reload()}catch(e){console.error(e),alert("Не вдалося видалити розробника.")}}//! SUBMIT ФОРМИ
...
const d={name:l.value,position:n.value};fetch(`${i}/${t}`,{method:"PATCH",body:JSON.stringify(d),headers:{"Content-Type":"application/json; charset=UTF-8"}}).then(s=>s.json()).then(s=>console.log("PATCH:",s)).catch(s=>console.log(s)),u()}//! MODAL
function u(){P.classList.toggle("is-hidden"),document.body.classList.toggle("no-scroll")}//! CONFIRM DELETE
function Q(e){e.preventDefault(),W.textContent=r,b.classList.remove("is-hidden"),document.body.classList.add("no-scroll")}function L(){b.classList.add("is-hidden")}function w(){l.value="",n.value="",p.src="/images/default.jpg",o=null,r="",u()}const V=document.querySelectorAll(".our-developers-list__img"),X=e=>{e.src=e.dataset.src,e.parentElement.querySelectorAll("source").forEach(s=>{s.srcset=s.dataset.srcset})},k=new IntersectionObserver(e=>{e.forEach(t=>{t.isIntersecting&&(X(t.target),k.unobserve(t.target),console.log(`Завантажено: ${t.target.alt}`))})},{threshold:.5});V.forEach(e=>k.observe(e));const Z=[{name:"Студія",href:"./index.html",active:!0},{name:"Портфоліо",href:"./portfolio.html",active:!1},{name:"Контакти",href:"#contacts",active:!1}],ee={links:Z},te=`{{#each links}}
