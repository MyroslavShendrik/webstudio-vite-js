import{H as d}from"./social-list-5012a507.js";const w=[{title:"УВАГА ДО ДЕТАЛЕЙ",text:"Ідейні міркування, і навіть початок повсякденної роботи з формування позиції."},{title:"ПУНКТУАЛЬНІСТЬ",text:"Завдання організації, особливо рамки і місце навчання кадрів тягне у себе."},{title:"ПЛАНУВАННЯ",text:"Так само консультація з широким активом значною мірою зумовлює."},{title:"СУЧАСНІ ТЕХНОЛОГІЇ",text:"Значимість цих проблем настільки очевидна, що реалізація планових завдань."}],h=` <li class="our-pluses-list__item">
  ...
  </li>`,j=document.querySelector(".our-pluses-list"),x=d.compile(h),L=w.map(e=>x(e)).join("");j.innerHTML=L;(()=>{const e={openModalBtn:document.querySelector("[data-modal-open]"),closeModalBtn:document.querySelector("[data-modal-close]"),modal:document.querySelector("[data-modal]")};e.openModalBtn.addEventListener("click",t),e.closeModalBtn.addEventListener("click",t);function t(){e.modal.classList.toggle("is-hidden"),document.body.classList.toggle("no-scroll")}})();const y=`<li class="our-developers-list__item">
...
const R=d.compile(y),p=o.map(e=>R(e)).join("");console.log(" markup:",p);U.innerHTML=p;console.log('%c Додаємо/Редагуємо/Видаляємо розробників в секції "Наша команда" ',"color: white; background-color: #D33F49");//! 1.1 Взяти з localStorage дані які вже у нас є! 
//! 1.2 Перетворити дані, які ми взяли з LocalStorage на масив об'єктів
//! 1.3 Записати це в якусь зміну
let s=JSON.parse(localStorage.getItem("dataDevelopers")),r=null;console.log("dataDevelopersList:",s);//! 2.1 Пошук всіх необхідних елементів
document.querySelector(".our-developers-list");const k=document.querySelector(".our-developers__button"),g=document.querySelector(".form-add-edit-developer__title"),D=document.querySelector("[data-modal-add-edit-developer]"),f=document.querySelector(".modal-add-edit-developer__button-delete"),n=document.querySelector(".form-add-edit-developer"),_=document.getElementById("button-cancel"),S=document.querySelectorAll("#img-developer"),i=document.querySelector(".form-add-edit-developer__button-submit"),c=document.querySelector(".form-add-edit-developer__img");console.log("imgFormDeveloper:",c);const u=document.querySelector(".form-add-edit-developer__input-developer-name"),v=document.querySelector(".form-add-edit-developer__input-developer-position");//! 2.2 Змінні для поточної роботи
//! 3 надати слухачів подій
k.addEventListener("click",q);n.addEventListener("submit",$);_.addEventListener("click",E);S.forEach(e=>{e.addEventListener("click",I),console.log("click on:",e)});f.addEventListener("click",M);//! 4 блок функцій 
//! Додає розробника
...
g.textContent="Додати розробника",i.textContent="Додати",f.style.display="none";//! 4.1.2 відкриття модального вікна з формою для збирання даних
...
g.textContent="Редагування розробника",i.textContent="Редагувати",f.style.display="block";//! при натисканні на розробника треба, щоб підставлялось його зображення
...
s=JSON.parse(localStorage.getItem("dataDevelopers")),console.log("dataDevelopersList:",s),console.log("event.target:",e.target),console.log("event.target.alt:",e.target.alt),console.log("event.target.name:",e.target.name),console.log("event.currentTarget:",e.currentTarget);const t=s.find(a=>a.name===e.target.alt);r=t.name,u.value=t.name,v.value=t.position,c.src=t.images.default,l()}function M(e){s=JSON.parse(localStorage.getItem("dataDevelopers")),console.log("dataDevelopersList 1:",s);//! знайти об'єкт
//! видалити об'єкт 
s=s.filter(t=>t.name!==r),console.log("dataDevelopersList 2:",s),localStorage.setItem("dataDevelopers",JSON.stringify(s,null,2)),l();//! перезавантажити сторінку \
...
function E(){l()}//! відкривання/закривання модального вікна для додавання/редагування розробника
function l(){console.log("відкривання/закривання модального вікна для додавання/редагування розробника"),D.classList.toggle("is-hidden"),document.body.classList.toggle("no-scroll")}//! підтвердження даних в формі модальних  вікон ДОДАТИ/РЕДАГУВАТИ 
...
const T=[{name:"Студія",href:"./index.html",active:!0},{name:"Портфоліо",href:"./portfolio.html",active:!1},{name:"Контакти",href:"./contacts.html",active:!1}],N={links:T},O=`{{#each links}}
