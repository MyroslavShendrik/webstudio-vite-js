...
const v=a.compile(u),n=o.map(e=>v(e)).join("");console.log(" markup:",n);w.innerHTML=n;console.log('%c Додаємо/Редагуємо/Видаляємо розробників в секції "Наша команда" ',"color: white; background-color: #D33F49");//! 1.1 Взяти з localStorage дані які вже у нас є! 
...
document.querySelector(".our-developers-list");const h=document.querySelector(".our-developers__button"),c=document.querySelector(".form-add-edit-developer__title"),j=document.querySelector("[data-modal-add-edit-developer]"),d=document.querySelector(".modal-add-edit-developer__button-delete"),i=document.querySelector(".form-add-edit-developer"),x=document.getElementById("button-cancel"),r=document.querySelectorAll("#img-developer");console.log("imgDeveloper:",r);//! 2.2 Змінні для поточної роботи
//! 3 надати слухачів подій
h.addEventListener("click",L);i.addEventListener("submit",y);x.addEventListener("click",R);r.forEach(e=>{e.addEventListener("click",U),console.log("click on:",e)});//! 4 блок функцій 
function L(){console.log("додаємо нового розробника");//! 4.1.1 заміна контенту заголовка модального вікна, приховуємо зайві елементи
c.textContent="Додати розробника",d.style.display="none";//! 4.1.2 відкриття модального вікна з формою для збирання даних
l()}function U(){console.log("Вікно редагування");//! 4.1.1 заміна контенту заголовка модального вікна, відкриваємо необхідні елементи
c.textContent="Редагування розробника",d.style.display="block",l()}//! Закривання модально вікна при стоворенні нового розробника 
function R(){l()}//! відкривання/закривання модального вікна для додавання/редагування розробника
...
function y(e){//! блокуємо перезавантаження сторінки 
...
l()}const k=[{name:"Студія",href:"./index.html",active:!0},{name:"Портфоліо",href:"./portfolio.html",active:!1},{name:"Контакти",href:"./contacts.html",active:!1}],_={links:k},D=`{{#each links}}
