...
const j=c.compile(w),d=o.map(e=>j(e)).join("");console.log(" markup:",d);h.innerHTML=d;console.log('%c Додаємо/Редагуємо/Видаляємо розробників в секції "Наша команда" ',"color: white; background-color: #D33F49");//! 1.1 Взяти з localStorage дані які вже у нас є! 
...
document.querySelector(".our-developers-list");const x=document.querySelector(".our-developers__button"),r=document.querySelector(".form-add-edit-developer__title"),L=document.querySelector("[data-modal-add-edit-developer]"),f=document.querySelector(".modal-add-edit-developer__button-delete"),a=document.querySelector(".form-add-edit-developer"),y=document.getElementById("button-cancel"),U=document.querySelectorAll("#img-developer"),l=document.querySelector(".form-add-edit-developer__button-submit"),n=document.querySelector(".form-add-edit-developer__img");console.log("imgFormDeveloper:",n);const R=document.querySelector(".form-add-edit-developer__input-developer-name"),k=document.querySelector(".form-add-edit-developer__input-developer-position");//! 2.2 Змінні для поточної роботи
//! 3 надати слухачів подій
x.addEventListener("click",_);a.addEventListener("submit",q);y.addEventListener("click",S);U.forEach(e=>{e.addEventListener("click",D),console.log("click on:",e)});//! 4 блок функцій 
//! Додає розробника
...
r.textContent="Додати розробника",l.textContent="Додати",f.style.display="none";//! 4.1.2 відкриття модального вікна з формою для збирання даних
...
r.textContent="Редагування розробника",l.textContent="Редагувати",f.style.display="block";//! при натисканні на розробника треба, щоб підставлялось його зображення
console.log("event.currentTarget:",e.target.currentTarget),console.log("typyof imgFormDeveloper:",typeof n);//! стратегічна задача (що робимо?)
...
s=JSON.parse(localStorage.getItem("dataDevelopers")),console.log("dataDevelopersList:",s),console.log("event.target:",e.target),console.log("event.target.alt:",e.target.alt),console.log("event.target.name:",e.target.name),console.log("event.currentTarget:",e.currentTarget);const t=s.find(m=>m.name===e.target.alt);R.value=t.name,k.value=t.position,n.src=t.images.default,i()}//! Закривання модально вікна при стоворенні нового розробника 
function S(){i()}//! відкривання/закривання модального вікна для додавання/редагування розробника
function i(){console.log("відкривання/закривання модального вікна для додавання/редагування розробника"),L.classList.toggle("is-hidden"),document.body.classList.toggle("no-scroll")}//! підтвердження даних в формі модальних  вікон ДОДАТИ/РЕДАГУВАТИ 
